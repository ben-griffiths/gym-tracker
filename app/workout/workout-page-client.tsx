...
function WorkoutPageContent({ routeSegment }: { routeSegment: string }) {
  ...
  /** URL session segment (skipped for `/workout/new` until replace). Bind early so chat cannot mint a duplicate session mid-rehydration. */
  ...
  const newRouteBootstrapRef = useRef(false);
  ...
  useEffect(() => {
    if (routeSegment !== "new") return;
    if (newRouteBootstrapRef.current) return;
    newRouteBootstrapRef.current = true;
    void ensureSessionId()
      .then((id) => {
        if (id) router.replace(`/workout/${id}`);
        else newRouteBootstrapRef.current = false;
      })
      .catch(() => {
        newRouteBootstrapRef.current = false;
      });
  }, [routeSegment, router]);
  ...
  const recentWorkouts = useMemo<RecentWorkoutSummary[]>(() => {
    ...
    for (const group of groups) {
      for (const session of group.sessions) {
        ...
        if (session.id === (sessionId ?? resumeSessionId)) continue;
...
export function WorkoutPageClient(props: { routeSegment: string }) {
  return <WorkoutPageContent {...props} />;
}
